 ...
 */
public class ReactiveMapperMethod {

  private final SqlCommand command;
  private final MethodSignature method;

  public ReactiveMapperMethod(Class<?> mapperInterface, Method method, Configuration config) {
...
//          executeWithResultHandler(sqlSession, args);
  ...
  private <E> Publisher<Object> executeForMany(ReactiveSqlSession sqlSession, Object[] args) {
    ...
    /*if (!method.getReturnType().isAssignableFrom(result.getClass())) {
      if (method.getReturnType().isArray()) {
        return result.collectList().map(this::convertToArray);
      } else {
        return result.collectList().map(it->convertToDeclaredCollection(sqlSession.getConfiguration(), it));
      }
    }*/
    return result;
  }
  ...
  public static class MethodSignature {
    ...
    private Class<?> returnActualGenericType;
